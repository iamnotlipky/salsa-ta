<?php 
	...
	if (isset($_POST['selesaiGet'])) {
		...
		$kem    = $_POST['kem'];
		if ($bayar == "" || $kem == "") {
			$response = ['response'=>'negative','alert'=>'Pastikan Membayar Terlebih Dahulu!'];
		}else{
			if ($bayar < $total) {
				$response = ['response'=>'negative','alert'=>'Transaksi Gagal, Uang Kurang!'];
			}else{
				$date  = date("Y-m-d");
            			...
            			<div class="card-body">
            				<form method="post">
								<div class="col-sm-12">
									...
									<button class="btn btn-dark" name="selesaiGet"><i class="fa fa-cart-plus"></i> Selesaikan Transaksi </button>
									<a href="?page=adminTransaksi" class="btn btn-danger"><i class="fa fa-repeat"></i> Kembali</a>
								</div>
							</form>
            			</div>
            		</div>
