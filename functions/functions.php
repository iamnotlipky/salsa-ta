...
class lsp
{
    public function login($username, $password)
    {
        ...
        if ($rows > 0) {
            if (base64_decode($assoc['password']) == $password) {
                return ['response' => 'positive', 'alert' => 'Berhasil Masuk!', 'level' => $assoc['level']];
            } else {
                return ['response' => 'negative', 'alert' => 'Kata Sandi Salah!'];
            }
        } else {

            return ['response' => 'negative', 'alert' => 'Nama Pengguna atau Kata Sandi Salah!'];
        }
    }

    public function redirect($redirect)
    {
        return ['response' => 'positive', 'alert' => 'Berhasil Masuk!', 'redirect' => $redirect];
    }
    ...
    public function register($kd_user, $name, $username, $password, $confirm, $foto, $level, $redirect)
    {
        ...
        if ($kd_user == " " || empty($kd_user) ||  empty($name) || $name == " " || empty($username) || $username == " " || empty($password) || $password == " ") {
            return ['response' => 'negative', 'alert' => 'Lengkapi Formulir!'];
        }
        ...
        if (strlen($username) < 6) {
            return ['response' => 'negative', 'alert' => 'Nama Pengguna Minimal 6 Huruf!'];
        }
        ...
        if ($rows == 0) {
            ...
            if ($password == $confirm) {
                ...
                $query   = mysqli_query($con, $sql);
                if ($query) {
                    return ['response' => 'positive', 'alert' => 'Pendaftaran Berhasil!', 'redirect' => $redirect];
                } else {

                    return ['response' => 'negative', 'alert' => 'Pendaftaran Gagal!'];
                }
            } else {

                return ['response' => 'negative', 'alert' => 'Kata Sandi Tidak Cocok!'];
            }
        } else if ($rows == 1) {

            return ['response' => 'negative', 'alert' => 'Nama Pengguna Telah Digunakan!'];
        }
    }
    ...
    function validateImage()
    {
        ...
        if (in_array($ekstensiBelakang, $ekstensi) === false) {
            return ['response' => 'negative', 'alert' => 'Gambar hanya boleh menggunakan ekstensi .jpg, .jpeg dan .png!'];
        }
