...
if (isset($_POST['btn-update'])) {
	...
	if ($_FILES['foto']['name'] == "") {
		...
		$response = $my->update("table_user", $values, "username", $_SESSION['username'], "?page=profil");
	} else {
		$response = $my->validateImage();
		if ($response['types'] == "true") {
			$value = "nama_user='$nama', foto_user='$response[image]'";
			$response = $my->update("table_user", $value, "username", $_SESSION['username'], "?page=profil");
		}
	}
}
	...
	if (mysqli_num_rows($exec) > 0) {
		if (base64_decode($asso['password']) == $password) {
			if (strlen($passwordbaru) < 6) {
				$response = ['response' => 'negative', 'alert' => 'Kata Sandi Minimal 6 Huruf!'];
			} else {
				if ($passwordbaru == $confirm) {
					...
					$response = ['response' => 'negative', 'alert' => 'Kata Sandi Berbeda!'];
				}
			}
		} else {
			$response = ['response' => 'negative', 'alert' => 'Kata Sandi Lama Salah!'];
		}
	}
}
?>
						...
						<div class="card">
							...
							<div class="card-body">
								<div class="form-group">
									...
									<input type="password" class="form-control form-control-sm" name="password" placeholder="Masukkan Kata Sandi Lama" required>
								</div>
								<div class="form-group">
									<label for="">Kata Sandi Baru</label>
									<input type="password" class="form-control form-control-sm" name="passwordbaru" placeholder="Masukkan Kata Sandi Baru" required>
								</div>
								<div class="form-group">
									<label for="">Konfirmasi Kata Sandi Baru</label>
									<input type="password" class="form-control form-control-sm" name="confirm" placeholder="Konfirmasi Kata Sandi Baru" required>
								</div>
								<hr>
								<button name="ubahPassword" class="btn btn-primary"><i class="fa fa-check"></i> Simpan</button>
							</div>
						</div>
					</form>
				</div>
