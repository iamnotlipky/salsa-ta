        ...
        if ($jumlah < 1) {
            ...
            if ($sisa['stok_barang'] < $jumlah) {
                ...
                if ($num > 0) {
                    ...
                    $value = "jumlah='$jumlah'";
                    $insert = $trans->update("table_pretransaksi", $value, "kd_transaksi = '$kd_transaksi' AND kd_barang", $barang, "?page=checkerTransaksi");
                    header("location:PageChecker.php?page=checkerTransaksi");
                } else {
                    $value = "'$kd_pretransaksi','$kd_transaksi','$barang','$jumlah','$total'";
                    $insert = $trans->insert("table_pretransaksi", $value, "?page=checkerTransaksi");
                    header("location:PageChecker.php?page=checkerTransaksi");
                }
            }
        }
...
if (isset($_GET['delete'])) {
    ...
    $response = $trans->delete("table_pretransaksi", $where, $id, "?page=checkerTransaksi");
}
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <?php if ($assoc2['count'] > 0 || isset($_POST['btnAdd'])) : ?>
                                <a class="btn btn-success" id="pembayaran" href="?page=checkerPembayaran">Lanjutkan ke pembayaran <i class="fa fa-cart-arrow-down"></i></a>
                            <?php endif ?>
                            <br><br>
                            <?php
                            $kr        = new lsp();
                                        ...
                                        <script>
                                            $("#btdelete<?php echo $no; ?>").click(function() {
                                                swal({
                                                    ...
                                                    if (isConfirm) {
                                                        window.location.href = "?page=checkerTransaksi&delete&id=<?= $dd['kd_pretransaksi']; ?>";
                                                    }
                                                })
                                            })
                                        </script>
                ...
                <table class="table table-hover table-bordered" id="sampleTable">
                    ...
                    <tbody>
                        <?php foreach ($barangs as $brs) { ?>
                            <tr>
                                <td><a href="pageChecker.php?page=checkerTransaksi&getItem&id=<?php echo $brs['kd_barang'] ?>"><?php echo $brs['kd_barang'] ?></a></td>
